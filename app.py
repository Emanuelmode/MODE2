...
Versión: v2.3 DELTA_VECTOR_FIX | Corrección δ vectorial + R3 conservador
...
def delta_mean(delta_obj):
    if isinstance(delta_obj, dict):
        vals = []
        for v in delta_obj.values():
            try:
                vals.append(float(v))
            except (TypeError, ValueError):
                pass
    ...
    try:
        return float(delta_obj)
    except (TypeError, ValueError):
        return np.nan

def delta_summary(delta_obj):
    if isinstance(delta_obj, dict):
        vals = []
        for v in delta_obj.values():
            try:
        ...
        if not vals:
            return "N/A"
        return f"μ={np.mean(vals):.4f} · {len(vals)} métr."
    return fmt(delta_obj, 4)
...
def fig_metrics(result):
    ...
    grads   = [float(sm[k].get("gradient", 0.0)) for k in present]
    stable  = [sm[k].get("stable", False) for k in present]
    deltas  = [float(sm[k].get("delta", 0.0)) for k in present]
    ...
    mx_grad = max(grads) if grads and max(grads) > 0 else 0.01
    mx_delta = max(deltas) if deltas and max(deltas) > 0 else 0.01
    x_max = max(mx_grad, mx_delta) * 1.35
    ax.set_xlim(0, x_max)

    for i, (bar, g, dk) in enumerate(zip(bars, grads, deltas)):
        y0 = bar.get_y()
        y1 = y0 + bar.get_height()
        ax.plot([dk, dk], [y0, y1], color=P["orange"], lw=1.8,
                label="δ por métrica" if i == 0 else None)
        ax.text(g + x_max*0.02, y0 + bar.get_height()/2,
                f"g={g:.7f}", va="center", ha="left", fontsize=6, color=P["text"])
        ax.text(min(dk + x_max*0.01, x_max*0.96), y0 + bar.get_height()/2 - 0.17,
                f"δ={dk:.7f}", va="center", ha="left", fontsize=5.5, color=P["orange"])
    ...
    ax2.text(0, -0.80, f"δ medio: {delta_mean(r3.get('delta')):.7f}", ha="center", fontsize=7, color=P["muted"])
    ...
    ax2.set_title("R3 Score — co-estabilización CONTINUA", fontsize=9, color=P["accent"])
...
def main():
    ...
    with st.sidebar:
        ...
        <b style="color:{P["text"]}">δ calibrado</b><br><br>
        Vectorial por métrica y régimen<br>
        (ver r3_delta_library.py)<br><br>
        <b style="color:{P["text"]}">Umbral coherencia</b><br>
        R3 ≥ 0.60 + mínimo interno</div>""", unsafe_allow_html=True)
                ...
                for start in range(0, len(xdata)-ws, wst):
                    ...
                    try:
                        ...
                        winrs.append({
                            ...
                            "delta_mean": float(r3.get("delta_mean", delta_mean(r3.get("delta")))),
                        })
                    except Exception:
                        pass
                    nw += 1
    ...
    c5.metric("δ medio",         delta_summary(r3.get("delta")))
    ...
    with tab5:
        ...
        rows = [
            ...
            ["δ medio",           delta_summary(r3.get("delta")),         "δ por métrica"],
