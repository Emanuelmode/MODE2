...
R3Descriptor: umbral coherencia conservador = 0.60
...
class RegimeDetector:
    ...
    def classify(self, lam: float, lz: float = None, d2: float = None) -> str:
        """
        Clasificación honesta priorizando que las señales sintéticas demo
        caigan en los regímenes esperables sin inflar R3.

        Idea central:
        - LZ sobre la proyección PCA-1 tiende a subestimar atractores clásicos
          como Lorenz y Rössler.
        - Por eso, cuando D2 ya sugiere atractor determinista de baja dimensión,
          λ actúa como desempate entre caos débil y caos clásico.
        """
        ...
        if not np.isnan(d2) and not np.isnan(lz):
            # Ruido puro / alta irregularidad
            if lz > 0.98 and d2 > 2.30:
                return 'noisy'

            # Hipercaótico reservado para firmas realmente extremas
            if lz > 0.94 and d2 > 2.25:
                return 'hyperchaotic'

            # Caos estructurado por complejidad alta clásica
            if lz > 0.55 and d2 > 1.60:
                return 'chaotic'

            # Desempate para atractores deterministas clásicos con LZ comprimida
            # por la proyección 1D del embedding.
            if 1.45 <= d2 < 2.00 and not np.isnan(lam):
                if lam >= 0.008:
                    return 'chaotic'          # Lorenz típico
                if lam >= 0.002:
                    return 'weakly_chaotic'   # Rössler típico

            # Periódico / casi-periódico de baja complejidad real
            if lz < 0.20 and d2 < 1.20 and (np.isnan(lam) or lam < 0.002):
                return 'stable'

            # Transiciones y cuasiperiodicidad
            if 1.20 <= d2 < 1.45 and not np.isnan(lam) and lam >= 0.002:
                return 'weakly_chaotic'

            # Zona de baja complejidad y baja dimensión
...
class R3Descriptor:
    ...
    def score(self, x: np.ndarray, tau: int, m: int = 3, epsilon_scalar: float = None) -> dict:
        ...
        # Embeddings casi degenerados: SampEn deja de ser una lectura fiable
        # del sistema y puede arrastrar artificialmente R3.
        ...
        for k, g in grads.items():
            ...
            if k == 'SampEn' and not sampen_reliable:
                stability_map[k] = {
                    'gradient': g,
                    'stable': False,
                    'delta': float(delta_map.get(k, 0.10)),
                    'weight': np.nan,
                    'w_compat': np.nan,
                    'used_in_score': False,
            ...
            # La compatibilidad de SampEn se conserva como diagnóstico,
            # pero no arrastra el score principal hasta validar mejor
            # su calibración sobre el mismo tipo de señal.
            w_compat = 1.0
            weight = w_stab
            if k == 'SampEn':
        ...
        if valid_n < 2:
            ...
            r3_dominant = min(
                (k for k, v in stability_map.items() if bool(v.get('used_in_score', False))),
                key=lambda k: stability_map[k]['weight']
            )

        # Coherencia conservadora: media ≥ 0.60 Y mínimo ≥ 0.30 Y no ruidoso.
        ...
        return {
            ...
            'R3_vector':     {
                k: (round(v['weight'], 8) if np.isfinite(v['weight']) else np.nan)
